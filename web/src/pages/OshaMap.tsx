/* ⚠⚠ 2026-09-12：這一頁已經【不在路由上】了，/osha 現在指向 OshaDistrictMap.tsx。

   原因不是這支寫得不好，是**真實資料畫不出逐筆的點**：
   職安法 74,658 筆裡只有 189 筆填了發生地點（0.25%），
   全台可用的座標大約 200 個。mock.osha.json 的 lat/lng 是亂數產生的，
   換成真實資料之後這一頁會變成一張幾乎空白的地圖。

   所以改成「以商工登記地址聚合到鄉鎮市區」的比例符號圖。
   這支的三樣東西被沿用到新版，沒有重寫：
     · 危害型態篩選（取代法條下拉選單）的設計
     · 「本筆公告涉及死亡災害」的措辭（不是「造成死亡」）
     · 行政救濟尚未終結要標示出來的規則

   這個檔案先留著不刪，因為：
     1. 如果之後拿到有座標的資料（例如職安署的職災調查報告），逐筆版就能直接復活
     2. 這是 T5 的工作成果，要留在 repo 裡當證據

   ⚠ 如果真的要讓這一頁復活：**下面那個 TileLayer 不能直接用**。
     tile.openstreetmap.org 上線後會回一堆「Access blocked / 403」
     （OSM 的圖磚是志工出錢跑的，有使用政策）。底圖改用
     src/data/tw_districts.json，做法看 OshaDistrictMap.tsx 的 GeoJSON 那一段。

   要改哪一頁之前先問一下，不要兩個人同時改同一份地圖。
   ============================================================ */
