...
#
# ⚠⚠ 「記帳及報稅代理人」「記帳及報稅代理業務人」一定要在這裡。
#   那個「及」是**業別名稱的一部分**，不是「甲及乙」的連接詞。
#   不認得它的話，`_MULTI`（會比對「及」）會把
#   「呂淑貞記帳及報稅代理業務人(呂淑貞)」判成「多名雇主」而整筆丟掉。
#
#   這不是假設，是人工標註抓出來的：T3 抽樣 200 筆裡有 50 筆拆不開，
#   兩個標註者對「拆不開算不算拆對」看法不同，追下去才發現全庫
#   **388 筆拆不開裡有 327 筆（84.3%）是這一個 pattern**。
#   修掉之後剩 61 筆。
#
#   ⚠ 只補這一類。其餘拆不開的多半是括號被來源截斷
#   （「六家加油站股份有限公司(羅美榮0」），那種**不可以猜**
#   —— 猜錯不是少一筆，是把錯的人名放進連結圖。見下面 _DUP_OPEN 的說明。
_ORG_INSIDE = re.compile(
    r"(股份有限公司|有限公司|國民小學|國民中學|榮民總醫院|附設醫院|"
    r"鄉公所|鎮公所|市公所|區公所|土木包工業|管理局|研究院|研究所|"
    r"財團法人|社團法人|醫院|大學|學校|教會|"
    r"記帳及報稅代理業務人|記帳及報稅代理人|記帳士事務所)"
)
