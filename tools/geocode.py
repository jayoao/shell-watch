...
⚠ 這三個百分比在 v1 的註解裡寫成 74.7 / 19.5 / 5.8，**分母算錯了**
  （拿 18,261+4,759+1,421 = 24,441 當分母，可是實際輸入是 24,819）。
  已重算。報告裡引用的是重算後的數字。
...
────────────────────────────────────────────────────────────────
⚠ 城鄉偏差是「變小」不是「消失」（2026-09-13 重算）
────────────────────────────────────────────────────────────────
修完那三個 bug 之後各縣市的失敗率（只列 ≥100 個地址的）：

    新竹市 37.6%   嘉義縣 18.7%   嘉義市 17.0%   雲林縣 14.8%
    花蓮縣 14.1%   苗栗縣 10.3%   宜蘭縣  8.2%   新竹縣  7.6%
    桃園市  7.4%   屏東縣  7.1%   台中市  7.1%   彰化縣  6.9%
    南投縣  6.8%   台南市  6.5%   高雄市  6.4%   基隆市  4.5%
    新北市  3.1%   台北市  0.4%

台北 0.4% 對 新竹市 37.6%，差了將近一百倍。**地圖上的點密度仍然
帶著這個偏差**：新竹市與嘉義、雲林、花蓮的點會比實際少。
畫面與報告都要寫明，不可以讓人把「點比較少」讀成「違規比較少」。
新竹市特別高的原因還沒查清楚（來源資料該縣市的門牌檔可能不完整），
這是待辦，不是已解的問題。
"""
